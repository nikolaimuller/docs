		...
		<Card>
			...
			>
				{({ tokens, getLineProps, getTokenProps }) => (
					<pre className="docs-overflow-scroll docs-bg-neutral docs-p-[1rem] docs-font-code selection:docs-bg-[color:var(--color-neutral-hover)] selection:docs-text-inherit">
						{tokens.map((line, key) => (
							<div
								key={String(key)}
								{...getLineProps({ line })}
							...
							>
								<div className="docs-table-cell">
									{line.map((token, key) => (
										<span key={String(key)} {...getTokenProps({ token })} />
									))}
								</div>
							</div>
						))}
